...
```bash
tools/build.sh
tools/run.sh --outdir examples/reference examples/6a-1hap_js12B.inp examples/pz74.inp
```
...
Both runs completed with no `ERROR` lines in the engine output. Repeated runs on
this machine reproduced both energies exactly.
